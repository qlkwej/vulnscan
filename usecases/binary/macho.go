...
func GetMachoInfo(command utils.Command, entity *entities.MachoInfo) (*entities.MachoInfo, error) {
	...
	switch header.Magic {
	case Magic32:
		entity.Bits = entities.Bits32
		entity.Endianness = entities.BigEndian
	case Magic64:
		entity.Bits = entities.Bits64
		entity.Endianness = entities.BigEndian
	case Cigam32:
		entity.Bits = entities.Bits32
		entity.Endianness = entities.LittleEndian
	case Cigam64:
		entity.Bits = entities.Bits64
		entity.Endianness = entities.LittleEndian
	default:
	...
	if cpu, ok := cpuTypes[header.Cpu]; ok {
		entity.Cpu = entities.CpuType(cpu)
		switch entity.Cpu {
		case entities.I386:
			if subCpu, ok := intelSubtype[header.SubCpu]; ok {
				entity.SubCpu = entities.SubCpuType(subCpu)
			}
		case entities.X8664:
